// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '["bfzy","zuida","bdzy","s360","lzi","guangsu","xinlang","jyzy","wujin","maotai","rycj","ffzy"]'); // 默认全选所有资源
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        ...
        const resp = await fetch('/api/sources', { signal: ctrl.signal });
    ...
    } catch (e) { /* 用 config.js 中已有的源 */ }

    // 每次启动都同步selectedAPIs为当前所有可用源
...
// 显示详情 - 修改为支持自定义API
let _showDetailsLoading = false;
async function showDetails(id, vod_name, sourceCode) {
    // 防止重复点击
    if (_showDetailsLoading) return;
    _showDetailsLoading = true;

    // 密码保护校验
    if (window.isPasswordProtected && window.isPasswordVerified) {
        if (window.isPasswordProtected() && !window.isPasswordVerified()) {
            showPasswordModal && showPasswordModal();
            _showDetailsLoading = false;
            return;
        }
    }
    if (!id) {
    ...
    try {
        ...
        const response = await fetch(`/api/detail?id=${encodeURIComponent(id)}${apiParams}${cacheBuster}`, { signal: controller.signal });
...
function playPreviousEpisode(sourceCode) {
    if (currentEpisodeIndex > 0) {
        ...
        const prevUrl = typeof prevEp === 'string' ? prevEp : (prevEp && prevEp.url ? prevEp.url : '');
...
function playNextEpisode(sourceCode) {
    if (currentEpisodeIndex < currentEpisodes.length - 1) {
        ...
        const nextUrl = typeof nextEp === 'string' ? nextEp : (nextEp && nextEp.url ? nextEp.url : '');
...
function renderEpisodes(vodName, sourceCode, vodId) {
    ...
    return episodes.map((episode, index) => {
        ...
        const epUrl = typeof episode === 'string' ? episode : (episode.url || '');
        return `
            <button id="episode-${realIndex}" onclick="playVideo('${epUrl.replace(/'/g, "\\'")}','${vodName.replace(/"/g, '&quot;')}', '${sourceCode}', ${realIndex}, '${vodId}')"
