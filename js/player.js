    ...
    constructor(config) {
        ...
        this.load = function (context, config, callbacks) {
            ...
            if (context.url && context.url.startsWith('http')) {
                context.url = '/player-proxy?url=' + encodeURIComponent(context.url);
            }
            // 恢复原始URL让HLS.js用CDN地址解析相对路径
            const origSuccess = callbacks.onSuccess;
            callbacks.onSuccess = function (response, stats, ctx) {
                ctx.url = originalUrl;
                return origSuccess(response, stats, ctx);
            };
...
function playEpisode(index) {
    ...
    const url = typeof ep === 'string' ? ep : (ep && ep.url ? ep.url : '');
...
async function testVideoSourceSpeed(sourceKey, vodId) {
    try {
        ...
        const firstEpisodeUrl = typeof firstEp === 'string' ? firstEp : (firstEp && firstEp.url ? firstEp.url : '');
        if (!firstEpisodeUrl) {
...
async function switchToResource(sourceKey, vodId) {
    ...
    try {
        ...
        // 获取目标集数的URL
        const targetEp = data.episodes[targetIndex];
        const targetUrl = typeof targetEp === 'string' ? targetEp : (targetEp && targetEp.url ? targetEp.url : '');
